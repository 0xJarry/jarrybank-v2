...
export async function GET() {
  try {
    ...
    if (rootMatch) {
      ...
      const varMatches = rootContent.matchAll(
        /\s*(--[^:]+):\s*([^;]+);/g
      );
      for (const match of varMatches) {
        ...
        if (!name.includes("font-") && !name.includes("radius") && 
            !name.includes("shadow-") && !name.includes("tracking-") && 
    ...
    if (darkMatch) {
      ...
      const varMatches = darkContent.matchAll(
        /\s*(--[^:]+):\s*([^;]+);/g
      );
      for (const match of varMatches) {
        const name = match[1].trim();
    ...
    return NextResponse.json(
      { error: "Failed to read CSS variables" },
      { status: 500 }
    );
  }
}
